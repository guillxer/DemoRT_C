...
int GraphicsBuffer::begin(void) {
  ...
  shadedBufferData=nullptr;
  shadedBufferData=(uint8_t *)malloc(_bufferSize);
  if(!bufferData && !shadedBufferData){
    return 1;
  }
  return 0;
}
